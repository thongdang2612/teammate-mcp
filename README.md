...
- `message_teammate` — post a message to a teammate over the backend's buffered SSE transport. Reads reply events up to `MESSAGE_TEAMMATE_WAIT_MS` (default 90s, kept under the ~100s proxy ceiling): returns `{status:"completed", reply}` for quick tasks, or `{status:"working", threadId}` for long ones (the run keeps going server-side in a detached forwarder) so the orchestrator isn't frozen holding an HTTP request open. Omit `teammateId` to continue an existing thread. Optional `attachmentUrls` are uploaded (S3 presign) before sending.
