...
- `message_teammate` — send a message to a teammate. The run is driven to completion by a **background job on the server** (via direct, un-proxied SSE calls, so long tasks aren't cut off by Diaflow's 30s MCP-proxy cap). A quick reply comes back **inline as plain text** within a short grace window; a longer one returns `{status:"working", jobId}` — the job keeps running server-side. Omit `teammateId` to continue an existing thread. Optional `attachmentUrls` are uploaded (S3 presign) before sending.
- `relay_teammates` — run a **sequential relay** across teammates (A→B→C…): the message goes to the first, its reply feeds the next, and so on. The whole chain runs as one background job (no per-step waiting from the caller). Input: `message` + `steps: [{ teammateId, instruction? }]` (each step's optional instruction is prepended to the previous step's output). Returns `{status:"working", jobId}`.
- `get_teammate_reply` — fetch the result of a background job by the `jobId` from `message_teammate`/`relay_teammates`. Returns the reply as **plain text** when done, `{status:"working", progress}` while it runs (call again shortly — it finishes server-side regardless), or an error if it failed. Unknown/expired jobId → `{status:"unknown"}`.

  > **Why background jobs:** Diaflow proxies every inbound MCP tool call with a hard **30s cap** (`modules/mcp/proxy.py`), and a chat orchestrator won't reliably poll for minutes. Doing the deterministic `await`/relay in a server-side job (like the Workflow Builder's flow engine, but inside the MCP) means the work completes regardless of how patiently the orchestrator checks back. Caveats: jobs are in-memory (lost on redeploy) and a Render free-tier spin-down can kill an in-flight job — keep the instance warm for long runs.
