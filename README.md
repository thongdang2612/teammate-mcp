...
- `message_teammate` — post a message to a teammate over the backend's buffered SSE transport. Reads reply events up to `MESSAGE_TEAMMATE_WAIT_MS` (default 20s — must stay under Diaflow's 30s MCP-proxy cap, beyond which the proxy discards the real result and injects a fake "too slow, don't retry" message): returns `{status:"completed", reply}` for quick tasks, or `{status:"working", threadId}` for long ones (the run keeps going server-side in a detached forwarder) so the orchestrator isn't frozen holding an HTTP request open. Omit `teammateId` to continue an existing thread. Optional `attachmentUrls` are uploaded (S3 presign) before sending.
- `get_teammate_reply` — after `message_teammate` returns `status:"working"`, call with the returned `threadId`. It reconnects to the run's event stream (`/threads/{id}/stream`, replay + follow) and waits server-side until a terminal event or the wait budget elapses, returning `{status:"completed", reply}` / `{status:"failed", error}` / `{status:"interrupted"}` / `{status:"working"}`. On `working` the run is still going — call again with the same `threadId` (unlimited). Falls back to `/threads/{id}/state` if the event buffer has expired.
