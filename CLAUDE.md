...
When `MCP_AUTH_MODE=oauth`, `src/index.ts`'s `buildHttpApp` takes a different branch entirely: `src/auth/oauth/wiring.ts` (`buildOAuthWiring`) mounts the MCP SDK's `mcpAuthRouter` (`/authorize`, `/token`, `/register`, `/revoke`, `.well-known` OAuth metadata) plus the magic-code `loginRouter` (`/login`), and protects `/mcp` with `requireBearerAuth` instead of the static inbound-token gate. The verifier behind both is `DiaflowOAuthProvider` (`src/auth/oauth/provider.ts`), which drives the same Diaflow magic-code login as the interactive flow but binds the resulting sealed session to the issued OAuth access token (in-memory `TokenStore`/`ClientStore`, per Task 6) rather than to a fixed `DIAFLOW_TOKEN` — so each authenticated caller gets its own per-user `ToolContext` (seal + workspace + rotation writeback into the token store) instead of sharing one service identity. Static mode (`MCP_AUTH_MODE` unset/`static`) is unchanged.
